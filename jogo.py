...
imagem_capa = pygame.image.load('CORRIDA FELIZ.png')
...
class Cavalo(pygame.sprite.Sprite):
    def __init__(self, imagens, x, y):
        super().__init__()
        self.imagens = [pygame.transform.scale(img, (int(img.get_rect().width * 0.04), int(img.get_rect().height * 0.04))) for img in imagens]
...
'''# Carregar as imagens de animação do cavalo do jogador
imagens_animacao_cavalo_jogador = [pygame.image.load(f'{i}.png').convert_alpha() for i in range(1, 5)]  # Ajuste o número de frames conforme necessário'''
...
nomes_arquivos_imagens = [('ma1.png'), 'ma2.png', 'ma3.png', 'ma4.png']
imagens_cavalos = [pygame.image.load(nome_arquivo_imagem).convert_alpha() for nome_arquivo_imagem in nomes_arquivos_imagens]
imagem.set_colorkey((0, 0, 0))  # Define a cor preta como transparente
...
imagem_colisao = pygame.image.load('poeira.png.png').convert_alpha()
